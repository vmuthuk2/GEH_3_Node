...
VPQ DPF_return3(mat Dl, cx_mat Z)
{
  VPQ dpf_return3;
  double bkva = 1000;
	double bkv = 0.48;
	...
	mat temp0 = zeros(1,6);
	temp0 <<  vo << 0 << vo << -120 << vo << 120 <<	arma::endr;;
	Vpolar = join_cols( temp0, Vpolar.rows(0, cnt_nodes -2));
	//cout << Vpolar << endl;
	//cout << "size of Vpolar: " << size(Vpolar) << endl;
	//cout << "size of V_nodes: " << size(V_nodes) << endl;//comment this
	//cout << "size of Iinj: " << size(Iinj) << endl;//comment this
	...
	dpf_return3.Vpolar = Vpolar;
	dpf_return3.PQb = PQb;
	dpf_return3.PQL = PQL;
	dpf_return3.Qset_a = Dl.col(7);
	dpf_return3.Qset_b = Dl.col(9);
	dpf_return3.Qset_c = Dl.col(11);
	return dpf_return3;
}
