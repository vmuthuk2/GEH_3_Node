...
void VVCAgent::vvc_slave()
{
  ...
  /*BOOST_FOREACH(CPeerNode peer, m_peers | boost::adaptors::map_values)
        {
            ModuleMessage mm = VoltageDelta(2, 3.0, "SSTI");
            peer.Send(mm);
...
//float sst_1 = device::CDeviceManager::Instance().GetNetValue("SST1", "AOUT/Ac_Pwr_Fb");
//std::cout<< "Real Power got from SST1 is " << sst_1 << std::endl;

//sst_1 = device::CDeviceManager::Instance().GetNetValue("SST1", "AOUT/Ac_Pwr_Rec_Fb");
//float sst_1 = device::CDeviceManager::Instance().GetNetValue("SST1", "AOUT/Active_Pwr_Fb");
//float sst_1 = device::CDeviceManager::Instance().GetNetValue("SST1/1", "AOUT/Active_Pwr_Fb");
float sst_1 = device::CDeviceManager::Instance().GetNetValue("LVSST1", "AOUT/Active_Pwr_Fb");
std::cout<< "Real Power got from SST1 is " << sst_1 << std::endl;
Pload << sst_1; //uncomment this
//Pload << P_LVSST1 << endr; //comment this
Pload = Pload.t();
std::cout << "Pload Value sent is: " << Pload << std::endl;
ModuleMessage mg_s = Gradient(Pload);
peer_master.Send(mg_s);
...
//bool status = Grad_slave1.load("Grad_slave1.mat");
  ...
  if (m_Synchronized)
  {
	  ...
	  std::cout << "Computed Command: " << sst1_cmd << std::endl;
	  m_Synchronized = 0;
	  //sst1_cmd = 0;
	  //std::cout << "actual sent command: " << sst1_cmd << std::endl;
...
try
{
	...
	//std::set<device::CDevice::Pointer> sst001; //Changed today
	sst000 = device::CDeviceManager::Instance().GetDevicesOfType("LVSST1");
	//sst001 = device::CDeviceManager::Instance().GetDevicesOfType("SST"); //Changed today
	//device::CDevice::Pointer dev0 = *sst000.begin(); //Changed today
	//device::CDevice::Pointer dev2 = *sst001.begin(); //Changed today
	//std::cout << "sst000 pointing to: " << dev0->GetID() << std::endl; //Changed today
	//std::cout << "sst001 pointing to: " << dev2->GetID() << std::endl; //Changed today  	
	//sst000 = device::CDeviceManager::Instance().GetDevicesOfType("SST1");
	//sst000 = device::CDeviceManager::Instance().GetDevicesOfType("SST1/1");
	if(!sst000.empty())
	{
		...
		float cmd_test;
		cmd_test = 314;
		//dev1->SetCommand("AIN/Re_Pwr", cmd_test);//
		//dev1->SetCommand("AIN/Reactive_Pwr_cmd", cmd_test);//
		dev1->SetCommand("AIN/Reactive_Pwr_cmd", sst1_cmd);//
	}
	else{
