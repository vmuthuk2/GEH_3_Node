...
float sst_1 = device::CDeviceManager::Instance().GetNetValue("LVSST2", "AOUT/Ac_Pwr_Fb");
std::cout<< "Real Power got from SST1 is " << sst_1 << std::endl;

sst_1 = device::CDeviceManager::Instance().GetNetValue("LVSST2", "AOUT/Active_Pwr_Fb");
std::cout<< "Real Power got from SST1 is " << sst_1 << std::endl;
Pload << sst_1; //uncomment this
//Pload << P_LVSST2 << endr; //comment this
Pload = Pload.t();
ModuleMessage mg_s = Gradient(Pload);
...
//send to mqtt
  ...
  if (m_Synchronized)
  {
	  ...
	  std::cout << "SST2 Command: " << sst2_cmd << std::endl;
	  m_Synchronized = 0;
...
try
{
    ...
    sst000 = device::CDeviceManager::Instance().GetDevicesOfType("LVSST2");
	...
	if(!sst000.empty())
	{
		...
		dev1->SetCommand("AIN/Reactive_Pwr_cmd", sst2_cmd);//
	}
	else{
